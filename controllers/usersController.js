...
const createUser = async (req, res) => {
    ...
    try {
        ...
        let urlPic;
        if (req.file)
            urlPic = await uploadCloud(`${req.destination}/${req.file.filename}`, body.uid, 'Social');
        ...
        if (req.file)
            await fs.unlink(`${req.destination}/${req.file.filename}`);
...
const updateUser = async (req, res) => {
    ...
    try {
        ...
        let urlPic;
        if (req.file)
            urlPic = await uploadCloud(`${req.destination}/${req.file.filename}`, uid, 'Social');
